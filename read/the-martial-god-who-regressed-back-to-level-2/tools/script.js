...
function injectBannerAd(adContainer, adIndex) {
    console.log('Injecting Adsterra ad for container:', adIndex);
    adContainer.innerHTML = '';

    // Add the Adsterra ad code
    adContainer.innerHTML = `
      <div id="adsterra-banner-${adIndex}">
        <script type="text/javascript">
          atOptions = {
            'key' : 'c1bb1b8c391a2e13a5e2d9a78d5d6599',
            'format': 'iframe',
            'height': 60,
            'width': 468,
            'params': {}
        ...
        <script type="text/javascript" src="//www.highperformanceformat.com/c1bb1b8c391a2e13a5e2d9a78d5d6599/invoke.js"></script>
      </div>
    `;
  }
...
async function loadComicImages() {
 ...
 // Add the banner ad container
 const adDiv = document.createElement('div');
 adDiv.className = 'banner-ad-container banner-ad';
 adDiv.style.minHeight = '60px'; // Match the ad height
 adDiv.style.width = '468px'; // Match the ad width
 adDiv.style.margin = '0 auto'; // Center the ad

 // Append both to the comic reader
 comicReader.appendChild(imageDiv);
 comicReader.appendChild(adDiv);

 // Inject the banner ad into the ad container
 injectBannerAd(adDiv, index);
 });
 } catch (error) {
