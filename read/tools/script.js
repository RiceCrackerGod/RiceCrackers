    ...
    .then(data => {
      ...
      if (elements.chapterList && manhwa.chapters?.length) {
        ...
        sortedChapters.forEach((chapter, index) => {
          ...
          div.style.cssText = `
            display: grid; grid-template-columns: 1fr 1fr; align-items: center; padding: 8px 10px; height: 66px;
...
function readFirstChapter() {
  ...
  fetch(apiUrl, { 
    ...
    .then(data => {
      const manhwa = data.manhwa_list.find(m => normalizeForMatching(m.title) === normalizedPageFilename);
      if (!manhwa || !manhwa.chapters?.length) return;

      // Sort chapters ascending and get the first one
      const firstChapter = [...manhwa.chapters]
        .sort((a, b) => a.chapter_number - b.chapter_number)[0];
      
      if (firstChapter) {
        window.location.href = `/read/${pageFilename}/chapter-${firstChapter.chapter_number}.html`;
      }
    })
    .catch(error => {
...
function readLatestChapter() {
  ...
  fetch(apiUrl, { 
    ...
    .then(data => {
      ...
      // Sort chapters descending and get the latest one
      const latestChapter = [...manhwa.chapters]
        .sort((a, b) => b.chapter_number - a.chapter_number)[0];
      
      if (latestChapter) {
        window.location.href = `/read/${pageFilename}/chapter-${latestChapter.chapter_number}.html`;
      }
    })
    .catch(error => {
